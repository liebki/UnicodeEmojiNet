...
public class EmojiManager(string operatingFolder, bool skipAdditional)
{
    ...
    public async Task DownloadAndProcessFiles()
    {
        ...
        foreach ((string urlstring, string name) data in _urls)
        {
            ...
            List<EmojiInfo> emojiList = ProcessHtmlFiles(HtmlFilePath);
            _allInfosList.AddRange(emojiList);
        }
    ...
    {
        Dictionary<string, HashSet<string>> mergedCategories = [];
        foreach (EmojiCategory category in _categoryList)
        {
            if (!mergedCategories.ContainsKey(category.CategoryName)) mergedCategories[category.CategoryName] = [];
    ...
    /// </summary>
    /// <param name="htmlFilePath">Path of the html source</param>
    /// <returns>The list with elements for the input source file</returns>
    private List<EmojiInfo> ProcessHtmlFiles(string htmlFilePath)
    {
        List<EmojiInfo> tempInfosList = [];
        ...
        foreach (HtmlNode row in rows)
        {
            if (row.InnerHtml.Contains("class='bighead'"))
            {
                ...
                if (tempSubCategoryList.Count == 0) emojiCategory = new EmojiCategory(lastMainCategory, []);
            }
            ...
            if (cells != null && cells.Count >= 2)
            {
                ...
                tempInfosList.Add(new EmojiInfo(emoji, name.Replace("\u229b ", ""), isRecentlyAdded, lastMainCategory,
                    lastCategory));
            }
